...
1. **定时生成**：
   ...
   - 日志文件以 `log_YYYYMMDD_HHmmss.json` 格式命名
...
- **本地存储**：`logs/YYYYMMDD/log.json`或`logs/YYYYMMDD/encrypted/log.json`（加密时）
- **远程存储**：
  - Gitee: 仓库中的`{upload_dir}/log_YYYYMMDD_HHmmss.json`
  - FTP: 服务器上的`{upload_dir}/log_YYYYMMDD_HHmmss.json`
